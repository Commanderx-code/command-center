...
function repoState(repo) {
  ...
  if (repo.ahead && repo.behind) return `↑ ${repo.ahead} · ↓ ${repo.behind}`;
...
function matchesFilter(repo, filter) {
  return filter === "all" || (filter === "dirty" && (repo.dirty || repo.ahead || repo.behind)) || (filter === "clean" && !repo.dirty && !repo.ahead && !repo.behind) || (filter === "ahead" && repo.ahead) || (filter === "behind" && repo.behind);
}
...
function populatePreferences(prefs) {
  for (const [id,key] of [["name","displayName"],["editor","editor"],["terminal","terminal"],["accent","accent"],["density","density"],["startup","startupPage"],["depth","scanDepth"],["refresh","refreshSeconds"]]) {
...
function bindEvents() {
  ...
  $("#preferences-form").addEventListener("input", () => $("#settings-status").textContent = "Unsaved changes");
