import { backupState, backupRecordFailed } from './feature-model.js';
export function backupOverview(health, integrations, now = Date.now()) {
  ...
  const mounted = report?.data?.drive_mounted===false ? false : report?.available ? report.data?.drive_mounted : undefined;
  ...
  const blocked = mounted === false;
  return {drive, state, completedAt: Number.isFinite(date) && report?.available && !backupRecordFailed(report?.data?.jobs?.backup) ? date : null,
...
export function renderBackupOverview({target, health, integrations, escapeHtml:e, desktop}) {
  ...
  const readiness=health?.backupReadiness;
  const jobRecords=health?.backup?.available?health.backup.data?.jobs:null;
  const records=Object.entries(jobRecords||{}).map(([name,record])=>`<li><strong>${e(name)}</strong>: ${e(record?.status||'Status not reported')} · ${e(record?.completed_at||'No completion time reported')}</li>`).join('');
  target.innerHTML=`<div class="backup-overview-grid"><article><small>Backup location</small><strong>${e(desktop?data.drive:'Desktop app required')}</strong><p>${e(integrations.resticRepository || 'Set a Restic repository in Settings')}</p></article><article><small>Last successful backup</small><strong>${e(data.completedAt?new Date(data.completedAt).toLocaleString():'No successful record')}</strong><p>${e(data.state.detail)}</p></article><article><small>Next step</small><strong>${e(data.reason)}</strong><p>${health?.checkedAt?`Checked ${e(new Date(health.checkedAt).toLocaleString())}`:'Refresh health to check your setup.'}</p></article></div><p class="settings-help">Drive state comes from your backup health helper. A connected drive does not verify repository integrity or credentials.</p><details open><summary>Backup readiness</summary><ul><li>Personal helper: ${health?.backupHelpers?.personal===true?'Available':'Unavailable or not checked'}</li><li>Full helper: ${health?.backupHelpers?.full===true?'Available':'Unavailable or not checked'}</li><li>Restic: ${readiness?.resticAvailable===true?'Available':'Unavailable or not checked'}</li><li>Location: ${e(readiness?.location||'Not checked')}</li><li>Credentials: ${e(readiness?.credentials||'Not checked')}</li></ul>${readiness?.healthAvailable===false?`<p>${e(readiness.healthError||'Backup health unavailable')}</p>`:''}<h4>Helper-reported job records</h4>${records?`<ul>${records}</ul>`:'<p>No usable job records. A missing check record is not a successful check.</p>'}<p>These Restic checks use Settings. Your backup helpers may use different repositories or credentials. Use “Test repository access” for an explicit authentication test; its result appears in Activity.</p></details>`;
