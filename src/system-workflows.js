export function serviceAuditGroup(row) {
  if(row.active==='failed')return 'Failed · investigate logs';
  if(row.active==='inactive'&&row.enablement==='enabled')return 'Enabled but inactive · review purpose';
  if(row.active==='inactive')return 'On-demand, disabled, or managed elsewhere';
  return row.active==='not loaded'?'Not loaded · usage unknown':'Active or transitioning';
}
export function filterServices(rows, query = '', status = 'all') {
  const words=query.toLowerCase().trim().split(/\s+/);
  return rows.filter(row => (status==='all'||row.active===status||(status==='review'&&row.active==='inactive'&&row.enablement==='enabled')) && words.every(word=>`${row.unit} ${row.description} ${row.active}`.toLowerCase().includes(word)));
}
...
export function createSystemWorkflows({state,$,escapeHtml:e,run,action,toast,switchView}) {
  ...
  function serviceList(){
    ...
    $('#services-list').innerHTML=rows.map(row=>`<button type="button" class="unit-row" data-unit="${e(row.unit)}"><strong>${e(row.unit)}</strong><span>${e(row.active)} · ${e(row.sub)} · ${e(row.enablement||'Unknown enablement')}</span><small>${e(serviceAuditGroup(row))}</small><small>${e(row.description)}</small></button>`).join('')||'<p class="empty-state">No matching services.</p>';
