...
export function createSystemWorkflows({state,$,escapeHtml:e,run,action,toast,switchView}) {
  ...
  function serviceList(){
    const words=$('#service-search').value.toLowerCase().trim().split(/\s+/);
    const rows=serviceRows.filter(row=>words.every(w=>`${row.unit} ${row.description} ${row.active}`.toLowerCase().includes(w)));
