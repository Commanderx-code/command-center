...
export function mountGitControls({ target, data, path, desktop, escapeHtml: e, action, refresh, toast, draft, invoke, jobs = [], openActivity }) {
  ...
  async function perform(kind) {
    ...
    busy = true; update();
    try {
      const id = await action({ action: kind, path, files: selected().map(file => file.path), branchName: kind === 'branch-create' ? $('#new-branch').value.trim() : $('#switch-branch').value, message: draft.message || '', indexTree: data.indexTree || '', expectedHead: data.head || '', branchRef: data.branchRef || '' }, () => {});
      running = Boolean(id);
