...
export function createPreferenceExtras({ state, $, escapeHtml: e, draftPreferences, populatePreferences, updateDraftStatus, action, toast, invoke }) {
  ...
  $('.settings-layout').insertAdjacentHTML('beforeend', `<section id="section-custom" class="panel settings-section"><div class="section-title"><span aria-hidden="true">✦</span><div><h3>Custom quick actions</h3><p>Pin commands to the dashboard. Every run is reviewed first.</p></div></div><div id="custom-actions-editor"></div><button type="button" class="secondary-button" id="add-custom-action">Add action</button><p class="settings-help">Direct commands use quoted arguments without shell expansion. Choose Fish for functions such as full-upgrade. Terminal modes support interactive prompts. Keep passwords out of saved commands and exports.</p></section>`);
  ...
  $('#dashboard-view').insertAdjacentHTML('beforeend', '<section class="panel module-panel"><div class="panel-heading"><div><p class="eyebrow">Shortcuts</p><h3>Your quick actions</h3></div><button type="button" class="secondary-button" id="manage-custom">Manage actions</button></div><div id="custom-action-buttons" class="button-row"></div></section>');
