...
fn start(mut command: Command) -> Result<(), String> {
    ...
    for _ in 0..10 {
        match child.try_wait().map_err(|e| e.to_string())? {
            ...
            Some(status) => {
                return Err(format!(
                    "Launcher exited with {status}; see the app terminal for details"
                ))
            }
...
fn configured(value: &str) -> Result<Vec<OsString>, String> {
    ...
    if words.is_empty() {
        return Err("Empty application command".into());
    }
...
fn terminal_command(words: &[OsString], path: &Path, child: &[OsString]) -> Command {
    ...
    match name.as_ref() {
        ...
        "gnome-terminal" | "xfce4-terminal" | "tilix" => {
        ...
        "wezterm" => {
            command.arg("start").arg("--cwd").arg(path);
        }
        _ => {} // Other terminals inherit the repository working directory.
    }
    if !child.is_empty() {
        match name.as_ref() {
            "xdg-terminal-exec" | "gnome-terminal" | "kitty" | "foot" | "wezterm" => {
                command.arg("--");
            }
            _ => {
                command.arg("-e");
...
fn terminal(path: &Path, child: &[OsString], preference: &str) -> Result<(), String> {
    if preference != "auto" {
        ...
        .contains(&preference)
        {
            return Err("Unsupported terminal preference".into());
        }
    ...
    if start(terminal_command(&["xdg-terminal-exec".into()], path, child)).is_ok() {
        return Ok(());
    }
        ...
        .contains("KDE")
    {
        for reader in ["kreadconfig6", "kreadconfig5"] {
            if let Ok(output) = Command::new(reader)
                .args([
                    "--file",
                    "kdeglobals",
                    "--group",
    ...
    Err(format!(
        "No terminal could be launched. Set TERMINAL to an installed terminal. {}",
        failures.join("; ")
    ))
}

fn desktop_file(id: &str) -> Option<PathBuf> {
    if !id.ends_with(".desktop") || id.contains('/') || id.contains('\\') {
        return None;
    }
...
fn editor(path: &Path, preference: &str, terminal_preference: &str) -> Result<(), String> {
        ...
        .output()
    {
        if output.status.success() {
            if let Some(entry) = desktop_file(String::from_utf8_lossy(&output.stdout).trim()) {
                ...
                if let Some(file) = editor_file(path) {
                    command.arg(file);
                }
                if start(command).is_ok() {
                    return Ok(());
                }
            }
        }
    ...
    for name in [
        ...
        if ["kate", "code", "codium", "zed"].contains(&name) {
            command.arg(path);
        } else if let Some(file) = editor_file(path) {
            command.arg(file);
        }
        if start(command).is_ok() {
            return Ok(());
...
pub fn open(
    path: &Path,
    target: &str,
    editor_preference: &str,
    terminal_preference: &str,
) -> Result<(), String> {
    if !cfg!(target_os = "linux") {
        return Err(
            "Automatic application detection currently supports Linux desktops only".into(),
        );
    }
    match target {
...
pub fn run_in_terminal(path: &Path, args: &[OsString], preference: &str) -> Result<(), String> {
    terminal(path, args, preference)
}
...
mod tests {
    ...
    fn quoted_editor_arguments_are_not_shell_code() {
        ...
        assert_eq!(
            words,
            vec!["kate", "--new-window", "/tmp/a b", "$(touch /tmp/never)"]
                .into_iter()
                .map(OsString::from)
                .collect::<Vec<_>>()
        );
    ...
    fn xdg_uses_monolithic_directory_and_literal_child_args() {
        let command = terminal_command(
            ...
            Path::new("/tmp/a b"),
            &["nvim".into(), "/tmp/a b".into()],
        );
        assert_eq!(
            command.get_args().collect::<Vec<_>>(),
            vec!["--dir=/tmp/a b", "--", "nvim", "/tmp/a b"]
        );
    }
    #[test]
    fn ghostty_uses_equals_for_directory() {
        let command =
            terminal_command(&["ghostty".into()], Path::new("/tmp/repo with spaces"), &[]);
        assert_eq!(
            command.get_args().collect::<Vec<_>>(),
            vec!["--working-directory=/tmp/repo with spaces"]
        );
    }
    #[test]
    fn konsole_uses_its_directory_flag() {
        {
            let (terminal, flag) = ("konsole", "--workdir");
