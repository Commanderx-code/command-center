...
use serde::Serialize;
use std::{collections::VecDeque, io::{Read, Write}, sync::{Arc, Mutex}, thread, time::{Duration, Instant}};
use tauri::{Manager, State};

const LIMIT: usize = 1_000_000;
#[derive(Default)]
struct Buffer { bytes: VecDeque<u8>, end: u64 }
impl Buffer {
    ...
    fn read(&self, cursor: u64) -> Output {
        ...
        let offset = if cursor_valid { (cursor - start) as usize } else { 0 };
        let bytes: Vec<_> = self.bytes.iter().skip(offset).take(65536).copied().collect();
        Output { cursor: start + offset as u64 + bytes.len() as u64, bytes, truncated: !cursor_valid, running: false }
    }
}
struct Session {
    id: String,
...
#[derive(Serialize)]
pub struct Output { bytes: Vec<u8>, cursor: u64, truncated: bool, running: bool }

impl Terminals {
    fn output(&self, id: &str, cursor: u64) -> Result<Output, String> {
        let state = self.0.lock().map_err(|e| e.to_string())?;
        let session = state.as_ref().filter(|s| s.id == id)
            .ok_or("Terminal output is no longer in memory. Activity retains the result only.")?;
        let mut output = session.buffer.lock().map_err(|e| e.to_string())?.read(cursor);
        output.running = session.running;
        Ok(output)
    }
    fn write(&self, id: &str, data: &[u8]) -> Result<(), String> {
        if data.len() > 8192 { return Err("Terminal input is too large; paste a smaller chunk".into()); }
        let mut state = self.0.lock().map_err(|e| e.to_string())?;
        let session = state.as_mut().filter(|s| s.id == id && s.running).ok_or("Terminal is not running")?;
        session.writer.as_mut().ok_or("Terminal input is closed")?.write_all(data).map_err(|e| e.to_string())
    }
    fn resize(&self, id: &str, cols: u16, rows: u16) -> Result<(), String> {
        ...
        let state = self.0.lock().map_err(|e| e.to_string())?;
        let session = state.as_ref().filter(|s| s.id == id && s.running).ok_or("Terminal is not running")?;
        session.master.as_ref().ok_or("Terminal is closed")?.resize(PtySize { rows, cols, pixel_width: 0, pixel_height: 0 }).map_err(|e| e.to_string())
    }
    fn signal(&self, id: &str, pid: Option<u32>, signal: i32) {
        if let Ok(state) = self.0.lock() {
            if let Some(master) = state.as_ref().filter(|s| s.id == id).and_then(|s| s.master.as_ref()) {
                if let Some(group) = master.process_group_leader().filter(|g| *g > 1) {
                    unsafe { libc::kill(-group, signal); }
                }
            }
        }
        if let Some(pid) = pid.filter(|p| *p > 1) { unsafe { libc::kill(-(pid as i32), signal); } }
    }
}

pub fn run(terminals: &Terminals, jobs: &Jobs, plan: &Plan, id: &str) -> (String, Option<i32>, String) {
    match execute(terminals, plan, id, || crate::jobs::cancelled(jobs)) {
        Ok((status, code)) => (status, Some(code), "Interactive workflow finished. Terminal output was not saved in Activity.\n".into()),
        Err(error) => ("failed".into(), None, format!("Could not run terminal: {error}")),
    }
}
fn execute(terminals: &Terminals, plan: &Plan, id: &str, cancelled: impl Fn() -> bool) -> Result<(String, i32), String> {
    ...
    *state = Some(Session { id: id.into(), master: Some(pair.master), writer: Some(writer), buffer: buffer.clone(), running: true });
    drop(state);
    ...
    thread::spawn(move || {
        ...
        while let Ok(n) = reader.read(&mut bytes) {
            if n == 0 { break; }
            if let Ok(mut buffer) = buffer.lock() { buffer.append(&bytes[..n]); }
        }
        let _ = done_tx.send(());
    });
    ...
    let result = loop {
        if cancelled() || start.elapsed() > Duration::from_secs(plan.timeout_seconds) {
            stopped = Some(if cancelled() { "cancelled" } else { "timed-out" });
    ...
    result.map(|status| (stopped.unwrap_or(if status.success() { "succeeded" } else { "failed" }).into(), status.exit_code() as i32)).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn terminal_read(terminals: State<Terminals>, id: String, cursor: u64) -> Result<Output, String> { terminals.output(&id, cursor) }
#[tauri::command]
pub async fn terminal_write(app: tauri::AppHandle, id: String, data: Vec<u8>) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || app.state::<Terminals>().write(&id, &data)).await.map_err(|e| e.to_string())?
}
#[tauri::command]
pub fn terminal_resize(terminals: State<Terminals>, id: String, cols: u16, rows: u16) -> Result<(), String> { terminals.resize(&id, cols, rows) }
#[tauri::command]
pub fn terminal_export(app: tauri::AppHandle, terminals: State<Terminals>, id: String) -> Result<String, String> {
    let state = terminals.0.lock().map_err(|e| e.to_string())?;
    let session = state.as_ref().filter(|s| s.id == id).ok_or("Output is no longer in memory")?;
    let buffer = session.buffer.lock().map_err(|e| e.to_string())?;
    let path = crate::platform::data_file(&app, &format!("toolbox-output-{}.txt", crate::platform::now()))?;
...
mod tests {
    ...
    fn interactive_tty_accepts_input_resizes_and_never_uses_activity_logs() {
        ...
        let start = Instant::now();
        while !terminals.output("test", 0).is_ok_and(|o| String::from_utf8_lossy(&o.bytes).contains("ready")) {
        ...
        assert!(text.contains("32 90") && text.contains("answer:hello"), "{text}");
    ...
    fn interactive_workflow_can_be_cancelled() {
        let terminals = Terminals::default();
        let plan = Plan::new("Test", "/bin/sh", vec!["-c".into(), "sleep 30 & wait".into()], std::path::Path::new("/tmp"));
        let start = Instant::now();
        let result = execute(&terminals, &plan, "stop", || start.elapsed() > Duration::from_millis(80)).unwrap();
